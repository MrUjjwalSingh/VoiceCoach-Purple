...
export default function DashboardPage() {
  ...
  const navItems = [
    ...
    { label: "Presentation Analysis", href: "/presentation-analysis", icon: FileText, active: false },
    { label: "Content Generator", href: "/content-generator", icon: BarChart2, active: false },
    { label: "Presentation Generator", href: "/presentation-generator", icon: PieChart, active: false },
    { label: "History", href: "/history", icon: Clock, active: false },
    { label: "Words Per Minute", href: "/wpm", icon: Activity, active: false },
  ]
  ...
  const wpm = 148
  const pacingStatus = wpm > 160 ? "Very fast" : wpm < 100 ? "Very slow" : "Within range"
  const strategicPauses = "Add short pauses after key points and before transitions to emphasize structure."
  const feedback = [
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-gradient-to-br from-[#1a0b2e] via-[#2d1b69] to-[#4c1d95] relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-background via-background to-primary/12" />
          ...
          <div className="flex flex-1">
            ...
            >
              ...
              <button
                ...
                className="absolute -right-4 top-8 w-8 h-8 bg-primary/10 backdrop-blur-md border border-primary/20 rounded-full flex items-center justify-center text-primary hover:bg-primary/20 transition-colors"
              >
                {isSidebarExpanded ? <ChevronLeft size={16} /> : <ChevronRight size={16} />}
              </button>

              <div className={`flex items-center gap-3 mb-8 ${!isSidebarExpanded && "justify-center"}`}>
            ...
            {/* Main Content */}
            <main className={`flex-1 p-8 transition-all duration-300 ${!isSidebarExpanded ? "pl-6" : ""}`}>
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <h2 className="text-2xl font-bold text-foreground mb-1">Dashboard Overview</h2>
                <p className="text-sm text-muted-foreground mb-6">Your analytics at a glance</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card className="p-6 bg-purple-500/10 backdrop-blur-sm border-purple-400/20">
                    ...
                    <div className="h-44">
                      <svg viewBox="0 0 200 80" className="w-full h-full">
                        ...
                        />
                        <polygon points="0,60 30,50 55,56 80,38 110,44 140,32 170,28 200,14 200,80 0,80" fill="url(#lg1)" />
                      </svg>
                    </div>
                  </Card>
                  ...
                  <Card className="p-6 bg-purple-500/10 backdrop-blur-sm border-purple-400/20">
                    ...
                    <div className="h-44 flex items-end gap-6 px-2">
                      <div className="flex-1 flex flex-col items-center">
                        <div className="w-12 bg-primary rounded-t-md" style={{ height: "52%" }} />
                        <div className="text-xs text-muted-foreground mt-2">Completed</div>
                      </div>
                      <div className="flex-1 flex flex-col items-center">
                        <div className="w-12 bg-secondary rounded-t-md" style={{ height: "78%" }} />
                        <div className="text-xs text-muted-foreground mt-2">Practiced</div>
                      </div>
                      <div className="flex-1 flex flex-col items-center">
                        <div className="w-12 bg-slate-400 rounded-t-md" style={{ height: "30%" }} />
                        <div className="text-xs text-muted-foreground mt-2">Sessions</div>
                      </div>
                    </div>
                  </Card>
                  ...
                  <Card className="p-6 bg-purple-500/10 backdrop-blur-sm border-purple-400/20">
                    ...
                    <div className="flex items-center gap-6">
                      <svg viewBox="0 0 36 36" className="w-28 h-28">
                        <circle r="16" cx="18" cy="18" fill="transparent" stroke="#ECEBFF" strokeWidth="8" />
                        <circle r="16" cx="18" cy="18" fill="transparent" stroke="#8B5CF6" strokeWidth="8" strokeDasharray="60 40" strokeLinecap="round" transform="rotate(-90 18 18)" />
                        <circle r="16" cx="18" cy="18" fill="transparent" stroke="#60A5FA" strokeWidth="8" strokeDasharray="24 76" strokeLinecap="round" transform="rotate(20 18 18)" />
                      </svg>
                      <div className="flex-1">
                        <ul className="space-y-2 text-sm">
                          <li className="flex items-center gap-3">
                            ...
                            <span className="text-muted-foreground ml-auto">42%</span>
                          </li>
                          <li className="flex items-center gap-3">
                            <span className="w-3 h-3 rounded-full bg-secondary inline-block" />
                            <span className="text-foreground">Growth</span>
                            <span className="text-muted-foreground ml-auto">28%</span>
                          </li>
                          <li className="flex items-center gap-3">
                            <span className="w-3 h-3 rounded-full bg-slate-400 inline-block" />
                            <span className="text-foreground">Innovation</span>
                            <span className="text-muted-foreground ml-auto">30%</span>
                          </li>
                        </ul>
                      </div>
                    </div>
                  ...
                  <Card className="p-6 bg-purple-500/10 backdrop-blur-sm border-purple-400/20">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="flex-1">
                        <div className="text-xs text-muted-foreground">Total WPM</div>
                        <div className="text-4xl font-bold text-foreground">{wpm}</div>
                        <div className="mt-3 flex items-center gap-3">
                          <span className={"px-2 py-1 text-xs rounded " + (pacingStatus === "Within range" ? "bg-green-600/15 text-green-300" : "bg-red-600/10 text-red-300")}>
                            {pacingStatus}
                          </span>
                          <span className="text-sm text-muted-foreground">• {strategicPauses}</span>
                        </div>
                      </div>
                ...
                <div className="mt-6">
                  <Card className="p-6 bg-purple-500/10 backdrop-blur-sm border-purple-400/20">
                    ...
                    <ul className="space-y-2 text-sm">
                      {feedback.map((fb, i) => (
                        <li key={i} className="p-3 bg-muted/5 rounded-md text-foreground">
                          {fb}
                        </li>
                      ))}
                    </ul>
